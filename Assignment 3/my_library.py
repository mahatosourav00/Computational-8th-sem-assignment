...
def shoting_method(x0, y0, zh0, zl0, xn, yn, h, fun1, fun2):#, file1, file2):
    yh,Xh,Yh = runge_kutta_4(x0, y0, zh0, h, fun1, fun2)
    yl,Xh,Yl = runge_kutta_4(x0, y0, zl0, h, fun1, fun2)
    if yh > yn and yl < yn:
        while abs(yh - yn) >= 0.0001 or abs(yl - yn) >= 0.0001:
        ...
        print("please change your 'zh' ",yh)
        return Xh,Yh
    elif yh > yn and yl > yn:
        Xh,Yh, print("please change your 'zl' ",yl)
        return Xh,Yh
    elif yh < yn and yl > yn:
...
def runge_kutta_4(x0, y0, z0, h, fun1, fun2, a=0, b=1):#, file):
    X = []
    Y = []
    Z = []
    x01 = x0
    ...
    while round(x0,6) <= b:
        ...
        k3y = h * fun1((x0 + h/2), (y0 + k2y/2), (z0 + k1z/2))
        k3z = h * fun2((x0 + h/2), (y0 + k2y/2), (z0 + k1z/2))
        k4y = h * fun1((x0 + h), (y0 + k3y), (z0 + k1z/2))
        k4z = h * fun2((x0 + h), (y0 + k3y), (z0 + k1z/2))
        z = z0 + (1/6 * (k1y + (2 * k2y) + (2 * k3y) + k4y))
        y = y0 + (1/6 * (k1z + (2 * k2z) + (2 * k3z) + k4z))
...
def monte_carlo_integral_prob_fun(a1,a2,fun_main,fun_try,N,alpha, a = 572, m = 16381):
    inte = 0
    for i in range(N):
        x = random_MLCG(a1, a2, a, m)
        prob = -math.log(1-x/alpha)
        inte += (fun_main(prob)/fun_try(prob,alpha))
    #print(inte,N)
    ans = ((a2-a1)/(N)) * inte
...
def monte_carlo_integral(a1, a2, fun, N, a = 572, m = 16381):
    inte = 0
